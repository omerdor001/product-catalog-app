...
4. **Search** — `GET /api/products/search/<query>?page=N` bypasses the cache entirely and forwards the query to DummyJSON's own search endpoint. The returned results are then paginated the same way as the main product listing, and the frontend renders the same Previous/Next controls for navigating through them.
...
- **Search is always live and paginated** — search results are never cached. The assumption is that search is infrequent enough that hitting DummyJSON each time is acceptable. The matching products are then sliced into pages of 15 locally, so the same pagination controls apply.
